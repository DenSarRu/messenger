...
from .db.database import ClientStorage
...
class Client(threading.Thread, metaclass=ClientVerifier):
    ...
    def __init__(self, address, port):
        ...
        self.__user_name, self.__password = self.__validate_username()
        self.__storage = ClientStorage(self.__user_name)
    ...
    def write_messages(self, sock, account_name):
        ...
        self.__storage.add_contact(receiver_name)
        self.__logger.debug(f'Сформировано сообщение: {message}')
        self.__storage.save_message(account_name, receiver_name, message)
    ...
    def message_from_server(self, user_name):
        ...
        while True:
            try:
                ...
                self.__logger.debug(f'Получено сообщение от сервера: {message}')
                if self.__CONFIGS.get('RESPONSE') in message:
                    ...
                    elif message[self.__CONFIGS.get('RESPONSE')] == 202:
                        return message
    ...
    @log
    def get_contact_list(self, login) -> list:
        contacts_list = []
        ...
        self.__logger.debug(f'Сформировано сообщение: {get_contacts_message}')
        send_message(self.__sock, get_contacts_message, self.__CONFIGS)
        response = self.message_from_server(login)
        contacts_list = response['alert']
        print(f"list of contacts: {contacts_list}")
        ...
        with socket_lock:
            ...
            try:
                ...
                if hanlded_response == "200 : OK":
                    ...
                    contact_list = self.get_contact_list(user_name)
                else:
                ...
                # Запускает отправку сообщений и взаимодействие с сервером
    ...
    def run(self):
        ...
        self.__logger.info(f'Запущен клиент с парамертами: '
                           f'адрес сервера: {self.__address}, порт: {self.__port}, имя пользователя: {self.__user_name}')
        self.create_client_socket(self.__user_name)

    @staticmethod
    def __validate_username() -> tuple:
        """
        ...
        while True:
            if user_name == 'Гость' or not user_name:
                ...
                except UsernameToLongError as err:
                    print(err)
                    exit(0)
            else:
                break
